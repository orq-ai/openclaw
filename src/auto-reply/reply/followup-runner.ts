...
import type { TypingMode } from "../../config/types.base.js";
...
export function createFollowupRunner(params: {
  ...
  return async (queued: FollowupRun) => {
    ...
    const diagnosticsEnabled = isDiagnosticsEnabled(run.config);
    const channel = effectiveQueued.originatingChannel ?? run.messageProvider?.toLowerCase();
    ...
    if (diagnosticsEnabled && run.sessionKey) {
      logMessageQueued({
