...
export async function runCronIsolatedAgentTurn(params: {
  ...
  let telemetry: CronRunTelemetry | undefined;
  {
    ...
    // calls runEmbeddedPiAgent directly and needs its own emission.
    if (cronDiagnosticsEnabled) {
      const usage = finalRunResult.meta?.agentMeta?.usage;
      const modelUsed =
        finalRunResult.meta?.agentMeta?.model ?? fallbackModel ?? liveSelection.model;
      const providerUsed =
        finalRunResult.meta?.agentMeta?.provider ?? fallbackProvider ?? liveSelection.provider;
      emitDiagnosticEvent({
        type: "run.completed",
        runId: cronSession.sessionEntry.sessionId ?? agentSessionKey,
        sessionKey: agentSessionKey,
        sessionId: cronSession.sessionEntry.sessionId,
        channel: "cron",
        provider: providerUsed,
        ...
        usage: {
          ...
          total: cronSession.sessionEntry.totalTokens,
        },
        durationMs: runEndedAt ? runEndedAt - runStartedAt : Date.now() - runStartedAt,
      });
    }
  }
