...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const agentSessionKey = prepared.context.agentSessionKey;
  ...
  try {
    ...
    if (isAborted()) {
      const aborted = prepared.context.withRunSession({
        status: "error",
        error: abortReason(),
      });
      emitCronRunProcessed(aborted);
      return aborted;
    }
    const finalOutcome = await finalizeCronRun({
    ...
    const errored = prepared.context.withRunSession({ status: "error", error: String(err) });
    emitCronRunProcessed(errored);
    return errored;
  }
}
